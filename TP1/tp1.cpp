...
int main()
{
	...
	{
		for (auto& pixelColor : imageInfo->pixels)
		{
			// 4a - A partir de l'image original, faite une transformation de sRGB a lineaire (faite la vraie transformation)
			const double normalisedColor = Algos::Normalize(pixelColor, HistInfo::MAX_COLOR_VALUE);
			const double linerizedColor = Algos::SrgbToLinear(normalisedColor);

			// 4b - En lineaire, appliquer une transformation constraste f(a) = a * 1.2 sur le result de 4a
			static constexpr double CONTRAST = 1.2;
			const double contrastedColor = linerizedColor * CONTRAST;
			...
			const double shinedColor = std::clamp(contrastedColor + SHINE, 0.0, 1.0);

			// 4d - Convertir le result de 4c en sRGB.
			const double srgbColor = Algos::LinearToSrgb(shinedColor);
			pixelColor = static_cast<uint8_t>(srgbColor * HistInfo::MAX_COLOR_VALUE);
		}
